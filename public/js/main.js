   ...
   'use strict'
        ...
        var CountDown = function() {
            var before = '<div class="square"><div class="numb">',
                ...
                /*if ($().countdown) {
                    $(".count-down").countdown('2019/01/08', function(event) {
        ...
        var showSuggestions = function() {
            $( ".top-search form.form-search .box-search" ).each(function() {
                $( "form.form-search .box-search input" ).on('keyup', (function() {
                    ...
                    $.ajax({
                         type:"GET",
                         url:"/getProduct/"+$('#search').val(),
                         success : function(results) {
                             $("#productSug").html('');
                             $.each(results, function (index, value) {
                                 $("#productSug").append('<li>\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t<div class="image">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<img src="storage/images/product/'+value.image.image1+'" alt="">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t</div>\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t<div class="info-product">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<div class="name">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<a href=\"/productDetails/'+value.slug+'\" title=\"\">'+value.productName+'</a>\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</div>\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<div class="price">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<span class="sale">\n' +
                                     '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t৳'+value.price+'\n' +
        ...
        var googleMap_s2 = function() {
            var data = JSON.parse('[{"address":"Quáº­n Bradford, Florida","content":"","status":"live"},{"address":"Quáº­n Bibb, Georgia","content":""},{"address":"Wateree Swamp, Nam Carolina","content":""}]');
		...
		var shipping = function() {
			$(document).ready(function(){
			  $("#shipping").hide();  // onload hide elements
			  $("#create-account-2").click(function(){
				  if($("#create-account-2").is(':checked'))
					$("#shipping").fadeIn("slow");  // checked
				  else
					$("#shipping").fadeOut("slow");  // unchecked
			  });
            });
